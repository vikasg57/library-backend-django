...
class BookAuthorMappingViewSet(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        data = BookHandler().create_books_and_author_map(
            book_name, description, year, author_names, user
        )
        return HttpResponse(json.dumps(data))
    ...
    def put(self, request, *args, **kwargs):
        ...
        data = BookHandler().update_books_and_author_map(
            book_name, description, year, book_id, user
        )
        return HttpResponse(json.dumps(data))
    ...
    def delete(self, request, *args, **kwargs):
        user = request.user
        book_id = request.GET.get("book_id")
        data = BookHandler().delete_books_and_author_map(book_id, user)
        return HttpResponse(json.dumps(data))
